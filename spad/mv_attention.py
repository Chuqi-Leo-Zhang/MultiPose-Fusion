...
class SPADTransformer(nn.Module):
    ...
    def forward(self, x, context=None, att_masks=None, plucker_embeds=None):
        return self.spad_forward(x, context=context, att_masks=att_masks, plucker_embeds=plucker_embeds)

    def spad_forward(self, x, context=None, att_masks=None, plucker_embeds=None):
        """
        ...
        # context = [context]
        context, plucker = [[context, att_masks]], plucker_embeds 
        ...
        return rearrange(x, "n v c h w -> n (v c) h w"), rearrange(x_in, "n v c h w -> n (v c) h w")  
